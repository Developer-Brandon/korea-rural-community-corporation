...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await openai.responses.create({
      model: 'gpt-4o',
      input: `한국투자증권의 AI 어시스턴트로서 "${query}"에 대해 최신 웹 정보를 검색하여 한국어로 답변해주세요. 구체적이고 도움이 되는 정보를 제공하고, 마크다운 포맷을 사용하여 가독성 있게 답변해주세요.`,
      tools: [
        {
          type: 'web_search',
        },
            ...
            messages: [
              {
                role: 'system',
                content:
                  '당신은 한국투자증권의 AI 어시스턴트입니다. 사용자의 질문에 대해 알고 있는 정보를 바탕으로 한국어로 답변해주세요.',
              },
              {
                role: 'user',
                content: query,
